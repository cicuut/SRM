...
class User(db.Model):
    ...
    def set_password(self, password):
        self.password_hash = bcrypt.hash(str(password)[:72])

    def check_password(self, password):
        return bcrypt.verify(str(password)[:72], self.password_hash)
...
class Patient(db.Model):
    ...
    role = db.Column(db.String(20), nullable=False, default="self")
...
class DeliveryRecord(db.Model):
    ...
    baby_complications = db.Column(EncryptedText, nullable=True)
    vit_k_given = db.Column(db.Boolean, nullable=False, default=False)
    hbo_given = db.Column(db.Boolean, nullable=False, default=False)
    imd = db.Column(db.Boolean, nullable=False, default=False)
    eye_ointment = db.Column(db.Boolean, nullable=False, default=False)
...
class VisitFamilyPlanning(db.Model):
    ...
    visit_kb_id = db.Column(PG_UUID(as_uuid=False), primary_key=True, default=generate_uuid)
    visit_id = db.Column(
    ...
    kb_id = db.Column(
        PG_UUID(as_uuid=False),
        db.ForeignKey("kb_record.kb_id"),
        nullable=False,
    )
    kb_method = db.Column(db.String(100), nullable=True)
